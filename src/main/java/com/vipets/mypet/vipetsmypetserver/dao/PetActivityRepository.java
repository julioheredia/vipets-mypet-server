...
public interface PetActivityRepository extends CrudRepository<PetActivity, Long> {
	...
	@Query("select p from PetActivity p where p.employed.userId = :userId and p.activityEnd >= :time order by p.clientScheduledTime ")
	List<PetActivity> petActivitysByEmployee(@Param("userId") Long userId, @Param("time") LocalDateTime time);
